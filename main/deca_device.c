...
{
	uint32_t i = (uint32_t)dwt_read32bitoffsetreg(DEV_ID_ID,0);
#if DEBUGMODE
	printf("A kiolvasott ertek = %x \n",i);
#endif
    return i;
}
...
#if DEBUGMODE
    printf("Belepett a dwt_readfromdevice-be, mindjart jon a readfromspi \n");
#endif
...
uint32 dwt_read32bitoffsetreg(int regFileID,int regOffset)
{
    ...
    if(result == DWT_SUCCESS)
    {
    	...
    	printf("Nem sikerult a read32bitoffsetreg-gel az olvasas \n");
    return regval ;
...
int dwt_spicswakeup(uint8 *buff, uint16 length)
{
    ...
    if(dwt_readdevid() != DWT_DEVICE_ID) // Device was in deep sleep (the first read fails)
    {
        ...
        printf("Tuljutott a dwt_readfromdevice-en \n");
        ...
        deca_sleep(5);
        printf("Tuljutott a dwt_sleep-en \n");
    }
    else
    {
        return ESP_OK;
...
#if DEBUGMODE
    printf("A statusz : %u 009_0_2_3_0 \n",(uint32_t)status);
#endif
    //NOTES:
...
#if DEBUGMODE
			printf("Megvolt a dwt_forcetrxoff 009_0_2_1 \n");
#endif
			dwt_rxreset();
#if DEBUGMODE
			printf("Megvolt a dwt_rxreset 009_0_2_2 \n");
#endif
...
#if DEBUGMODE
				printf("Megvolt a dwt_write16bitoffsetreg 009_0_2_3");
#endif
			}
			else
			{
    ...
    if(status & SYS_STATUS_RXFCG) // Receiver FCS Good
	{
		if(status & SYS_STATUS_LDEDONE) // LDE done/finished
		{
			...
			if (status & SYS_STATUS_RXOVRR) // NOTE when overrun both HS and RS pointers point to the same buffer
			{
				...
				dwt_forcetrxoff();
#if DEBUGMODE
			printf("Megvolt a dwt_forcetrxoff 009_0_2_4 \n");
#endif
				dwt_rxreset();
#if DEBUGMODE
			printf("Megvolt a dwt_rxreset 009_0_2_5 \n");
#endif
				if(dw1000local.sysCFGreg & SYS_CFG_RXAUTR) // Re-enable of RX is ON, then re-enable here (ignore error)
				{
					dwt_write16bitoffsetreg(SYS_CTRL_ID,0,(uint16)SYS_CTRL_RXENAB);
#if DEBUGMODE
			printf("Megvolt a dwt_write16bitoffsetreg 009_0_2_6 \n");
#endif
				}
...
#if DEBUGMODE
			printf("Megvolt a dwt_read16bitoffsetreg 009_0_2_7 \n");
#endif

                dwt_readfromdevice(RX_BUFFER_ID,0,2,dw1000local.cdata.fctrl) ;
#if DEBUGMODE
			printf("Megvolt a dwt_readfromdevice 009_0_2_8 \n");
#endif
...
#if DEBUGMODE
			printf("Megvolt a dwt_readfromdevice 009_0_2_9 \n");
#endif
                       ...
                       ((buff & (SYS_STATUS_HSRBP >> 24)) << 1)) // Host Side Receive Buffer Pointer
                    {
                        ...
                        dwt_write32bitreg(SYS_STATUS_ID,clear); // Write status register to clear event bits we have seen
#if DEBUGMODE
			printf("Megvolt a dwt_write32bitreg 009_0_2_10 \n");
#endif
                    }
                    ...
                    if((dw1000local.sysCFGreg & SYS_CFG_RXAUTR) == 0) // Double buffer is on but no auto RX re-enable RX
                    {
                        dwt_write16bitoffsetreg(SYS_CTRL_ID,0,(uint16)SYS_CTRL_RXENAB) ;
#if DEBUGMODE
			printf("Megvolt a dwt_write32bitreg 009_0_2_11 \n");
#endif
                    }
...
#if DEBUGMODE
			printf("Megvolt a dwt_writetodevice 009_0_2_12 \n");
#endif
                    }
                    else
                    {
                        ...
                        dwt_forcetrxoff();
#if DEBUGMODE
			printf("Megvolt a dwt_forcetrxoff 009_0_2_13 \n");
#endif
                        dwt_rxreset();
#if DEBUGMODE
			printf("Megvolt a dwt_rxreset 009_0_2_14 \n");
#endif
                        if(dw1000local.sysCFGreg & SYS_CFG_RXAUTR) // Re-enable of RX is ON, then re-enable here
                        {
                            dwt_write16bitoffsetreg(SYS_CTRL_ID,0,(uint16)SYS_CTRL_RXENAB) ;
#if DEBUGMODE
			printf("Megvolt a dwt_write16bitoffsetreg 009_0_2_15 \n");
#endif
                        }
                    }
...
void dwt_setleds(uint8 test)
{
    ...
    if(test & 0x1)
    {
        ...
        buf[1] |= 0x14;
        printf("A kirando ertek a led regiszterre (0x26) : %u \n",buf[1]);
        dwt_writetodevice(GPIO_CTRL_ID,0x01,1,&buf[1]);
        dwt_readfromdevice(GPIO_CTRL_ID,0x01,1,&datareaded);
        printf("A kiirt ertek a led regiszterre (0x26) : %u \n",datareaded);
