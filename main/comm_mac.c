...
static rx_result_t handle_rx_packet(mac_packet_info_t *mac_pkg)
{
    ...
    if(type == MAC_FRAME_TYPE_RANGING)
    {
        ...
        {
            printf("RX, RTLS packet process failed: %d", ret);
            dwt_rxenable(0);
        }
...
static int mac_send_ranging_message(rtls_struct_t* rtls)
{
    if(rtls->out_length > 0)
    {
        ...
        if(rtls->tx_ts_32 != 0)
        {
            ...
            if(r != DWT_SUCCESS)
            {
                printf("RTLS message TX failed: %d ", (rtls->out[0]));
                return -1;
            }
        }
        else
...
{
#if DEBUGMODE
	printf("Belepett a mac_receive_poll-ba 009_0_3_1 \n");
#endif
